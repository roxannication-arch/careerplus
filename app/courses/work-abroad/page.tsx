...
const lessons = [
  {
    ...
    body:
      "Ты отправляешь резюме и получаешь тишину. Думаешь, это с тобой что-то не так. Не с тобой. Расскажу, как реально устроена воронка поиска работы за рубежом — и почему 300–500 откликов ради оффера это норма, а не провал."
  },
  {
    title: "Выбрать рынок",
    body:
      "Половина людей бросает поиск, потому что целятся не туда — туда, где их не возьмут. Разберём, где реально есть путь для твоей профессии, а где упрёшься в стену."
  },
  {
    title: "Что ты продаёшь на мировом рынке",
    body:
      "Твой опыт зарубежному работодателю ничего не говорит. Не потому что плохой — потому что упакован под твой текущий рынок. Покажу, как переупаковать так, чтобы тебя поняли и захотели купить."
  },
  {
    title: "Резюме под международный стандарт",
    body:
      "70% резюме не доходят до человека — их отсеивают системы отбора. Разберём, как написать так, чтобы пройти фильтры роботов и попасть к рекрутёру. На выходе — готовое резюме."
  },
  {
    title: "LinkedIn под зарубежных рекрутёров",
    body:
      "Можно быть отличным специалистом и при этом невидимым для зарубежных рекрутёров. Headline, About, ключевые слова — как стать видимым в поиске тех, кого ты хочешь, чтобы нашли."
  },
  {
    title: "Поиск без рассылок + рефералки",
    body:
      "Массовая рассылка не работает — точечный поиск и рефералки работают в разы лучше. Один реферал = 200 холодных откликов."
  },
  {
    title: "Интервью на английском",
    body:
      "Поведенческие вопросы, культурные коды и главное: как обсуждать спонсорство визы так, чтобы тебя не слили после второго звонка."
  },
  {
    title: "Оффер и деньги",
    body:
      "Большинство принимают первый оффер. Это +20–40% к зарплате, которые ты теряешь. Разберём переговоры, что обсуждать кроме базы (RSU, бонус, sign-on), и обзор вариантов оплаты — без серых схем."
  }
];

const bonuses = [
  "Шаблоны международного резюме",
  "Шаблоны сообщений для outreach",
  "Чек-лист оффера",
  "Переговорный скрипт"
];
...
const faq = [
  ...
  },
  {
    ...
    answer:
      "Оплата идёт через Whop. Доступные способы зависят от платформы и вашей страны."
  },
  {
    question: "Можно ли задать вопрос перед покупкой?",
    answer: "Если хотите уточнить, подходит ли вам курс, напишите нам до оплаты — подскажем по вашей ситуации и формату курса."
  },
  {
  ...
  },
  {
    ...
    answer: `Курс будет доступен ${courseAccessDate}.`
  }
];
            ...
            {lessons.map((lesson, index) => (
              <details key={lesson.title} className="group rounded-[1.5rem] border border-ink/20 bg-white p-5 open:border-accent md:p-6">
                <summary className="flex cursor-pointer list-none items-start justify-between gap-6">
                  <div>
                    <p className="text-sm font-semibold text-accent">Урок {index + 1}</p>
      ...
      <section className="px-5 py-6 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-5xl rounded-[1.75rem] border border-ink/20 bg-white p-8">
          ...
          <ul className="mt-6 grid gap-3 text-sm leading-6 text-muted md:grid-cols-2">
            ...
            <li>Не юридическая или налоговая консультация — финансовые и правовые вопросы лучше проверять со специалистом</li>
