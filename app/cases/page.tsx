      ...
      <section className="py-20">
        <Container>
          <div className="grid gap-6">
            {cases.map((item) => (
              <article key={item.name} className="rounded-2xl border border-border bg-secondary p-8">
                <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                  <div>
                    <p className="text-sm font-semibold text-accent-hover">{item.market}</p>
                    <h2 className="mt-3 text-2xl font-semibold tracking-tight text-white">{item.name}</h2>
                  </div>
                  <div className="flex h-14 w-14 items-center justify-center rounded-full border border-border text-sm font-semibold text-white">
                    C+
                  </div>
                </div>
                <div className="mt-8 grid gap-4 lg:grid-cols-3">
                  <div className="rounded-2xl border border-border bg-primary/50 p-5">
                    <h3 className="text-sm font-semibold text-white">Было</h3>
                    <p className="mt-3 text-sm leading-6 text-muted">{item.before}</p>
                  </div>
                  <div className="rounded-2xl border border-border bg-primary/50 p-5">
                    <h3 className="text-sm font-semibold text-white">Сделали</h3>
                    <p className="mt-3 text-sm leading-6 text-muted">{item.work}</p>
                  </div>
                  <div className="rounded-2xl border border-border bg-primary/50 p-5">
                    <h3 className="text-sm font-semibold text-white">Стало</h3>
